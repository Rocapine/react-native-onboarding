...
- `nestedFillLayout(p)` / `fillLayout(fills)` — the **fill contract** for every box below it: `flexGrow: 1` to fill an outer box that got a definite main size (only if the element asked for flex sizing at all), plus `flexShrink: 1` so it is still **clamped** by that box. `flexBasis: 0` was doing both jobs — collapsing the box (the bug) *and* clamping it — so dropping it without the shrink lets a nested box grow to its own content and overflow the wrapper: that is how a wrapped `ScrollView` loses its bounded height and stops scrolling. **Never `flex` on a nested box.**
