...
A `backgroundGradient` forks the renderer into a gradient branch that nests content inside `<GradientBox>` — the outer wrapper carries the box layout (`flex`/`height`/etc.) and an **inner** view fills it. The trap: the inner view used a hardcoded `flex: 1`, but the non-gradient path is content-sized. When the box has no explicit `height`/`flex`, the outer wrapper is content-sized too, so the inner `flex: 1` instead grabs the **parent's** full main-axis — inside a `ZStack`/flex container the element balloons to fill the whole screen (v1.44.7 bug: gradient `Button`/`SafeAreaView`/`KeyboardAvoidingView`/`ScrollView`). Gate any inner fill on an explicit size with the **imported** predicate — `import { fillsParent, fillLayout } from "./wrapperLayout"`, then `const fills = fillsParent(p)` and spread **`fillLayout(fills)`**. Never hand-roll `p.height != null || p.flex != null || p.flexGrow != null` (a private copy that lost the `flexGrow` term is how `RiveElement` came to force every wrapped flexed Rive square), and never write a literal `flex: fills ? 1 : …`. `flex` implies `flexBasis: 0`, so a nested `flex: 1` measures 0 as soon as the outer box's own main size is auto, which is #231 one level down; `fillLayout` emits `flexGrow: 1` + `flexShrink: 1` instead (fill AND stay clamped). Wired that way in `ScrollViewElement`, `SafeAreaViewElement`, `KeyboardAvoidingViewElement`, `ButtonElement` (both gradient views) and `CarouselElement`'s measuring box; `Runtime/__tests__/wrapperLayout.test.ts` fails a renderer that reintroduces the literal. Two inner `flex: 1`s are deliberately left: `DrawingPadElement`'s `styles.fill` (its container's height is `props.height ?? 200`, always definite) and `ProgressIndicatorElement`'s track (a horizontal fill in a definite-width row, with its own explicit height) — neither can meet an auto-sized outer.

**A concrete instance, measured, because this fork hid it for as long as it existed:** a `Text` with `flex` *and* `backgroundGradient` rendered an **empty gradient pill** — the label was completely invisible. Its inner `<Text>` carried the parent-facing `flex` (so `flexBasis: 0` → the text box measured 0) while the `GradientBox` took its height from padding alone, so nothing was line-determined and nothing reserved space for the glyphs. Read off the accessibility tree, the two `Text` nodes had **no frame at all** before and `h=16.0` after, with everything below the row 16pt higher — one line height. Before/after: `~/Developer/.parity-artifacts/pr-231/ios-41-before-gradient-text.png` and `ios-42-after-gradient-text.png`. Two reviews disagreed about whether this could be visible (one argued both boxes zero together and the outer's size is line-determined); the measurement settled it, and the case now lives permanently on the guards example screen.
