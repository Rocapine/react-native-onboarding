...
| `RadioGroup` / `CheckboxGroup` | `items: [{label?,value,subLabel?}]` (label optional; `subLabel` = secondary line); tick styling via `tickPosition`/`tickColor`/`tickSelectedColor`/`tickBorderRadius`/`tickSize`; sub-label styling via `itemSubLabel*` | `options`; a non-unique `value` |
