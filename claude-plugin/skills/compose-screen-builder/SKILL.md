...
- `{ "type": "custom", "function": "name", "variables": ["a","b"], "onResolve": [...], "onError": [...], "retry": {"maxAttempts": 3, "delayMs": 500} }` — emit to host. Host wires the implementation. The handler is awaited; put the gate's `"continue"` in `onResolve` (after the action it would advance on failure too), and error UI in `onError` (terminal — the rest of the list is skipped). `actions.pending.<elementId>` reads `"true"` while the list runs, for a `renderWhen`-gated spinner or a `disabledWhen` CTA (inside a `Repeat`, spell it with the TEMPLATE id). Never leave the screen's only `"continue"` in `onResolve` alone — a throw, or a `function` the app never registered, then strands the user.
