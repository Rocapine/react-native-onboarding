...
| `ProgressiveBlurImage` | `url`, `intensity` (0–100), `mask` (linear `{from,to,stops}` or radial `{type:"radial",center?,radius?,stops}`), optional `tint`/`maxBlurOpacity` | `blurRadius` (that's `Image`); a color `mask` (mask opacity = blur strength, not a color) |
