...
Runtime **ignores `onPress`** on elements that already own a tap/focus/scroll gesture: `Button` (use `actions`), `RadioGroup`, `CheckboxGroup`, `DatePicker`, `Input`, `WheelPicker`, `Slider`. The schema still accepts it there (it's on `BaseBoxProps`), so don't rely on it for those — wire selection/continue via their own props instead.
...
- This screen's `Input` / `RadioGroup` / `CheckboxGroup` / `DatePicker` / `WheelPicker` / `Slider` `variableName` prop
