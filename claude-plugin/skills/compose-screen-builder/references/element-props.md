...
| `Carousel` | yes | `carouselType` · `autoPlay` · `autoPlayInterval` · `loop` · `showDots` · `dotColor` · `activeDotColor` · `dotWidth` · `dotHeight` · `activeDotWidth` · `activeDotHeight` · `dotsGap` · `dotsPosition` · `dotsMarginTop` · `dotsMarginBottom` · `defaultIndex` · `variableName` |
...
| `TypewriterText` | — | `content` · `mode` · `preset` · `duration` · `delay` · `stagger` · `easing` · `spring` · `loop` · `loopDelay` · `cursor` · `cursorChar` · `fontSize` · `fontWeight` · `fontFamily` · `fontStyle` · `color` · `textAlign` · `letterSpacing` · `lineHeight` |
