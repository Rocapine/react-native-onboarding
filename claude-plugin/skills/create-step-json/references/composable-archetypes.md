...
| `DrawingPad` | — | freehand draw/signature canvas; serializes per stroke into `variableName` (SVG path string) and/or `imageVariableName` (base64 image data URI — set either or both); `strokeColor`, `strokeWidth` (>0, default 2), `backgroundColor`, `clearable` (default true), `imageFormat: "png"\|"jpeg"` (default `"png"`); box defaults height 200 / borderWidth 2 / borderRadius 16 / borderColor primary; needs `@shopify/react-native-skia`; see `DrawingPadElement.ts` |
