...
| `TypewriterText` | — | reveals `content` one character at a time, each char mounting its own reanimated entering animation (per-char delay = `delay + charIndex * stagger`) — the staggered-headline reveal, **never writes a variable**. `content` (required), `mode` (`"plain"`/`"expression"` for `{{var}}`), `preset` (entering preset, default `"FadeInDown"`), `duration` (400), `delay` (0), `stagger` (45 — the typewriter knob), `easing`, `spring: {damping?,stiffness?,mass?}` (wins over `easing`), plus text styling. Distinct from the whole-block `animation.entering` (fades the string at once) and from `AnimatedText` (an animated number); wraps by whole words; leaf, non-interactive; see `TypewriterTextElement.ts` |
