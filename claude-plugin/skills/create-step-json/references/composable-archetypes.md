...
> **Schema reminder.** `payload = { elements: UIElement[] }`. No `payload.root`, no `payload.variables`. Variables flow at runtime from in-screen `Input` / `RadioGroup` / `CheckboxGroup` / `DatePicker` / `WheelPicker` / `Slider` `variableName` props (and `DrawingPad`'s `variableName` / `imageVariableName`) (or prior steps' captures).
...
Every element also accepts optional `onPress: ButtonAction[]` (also `BaseBoxProps`) — the same action list as `Button.actions` (`"continue"` / `{type:"setVariable",…}` / `{type:"custom",…}`), making any element tappable (e.g. a tappable `Image` card or `YStack` row). Runtime ignores it on elements that own their own gesture (`Button` — use `actions`; `RadioGroup`, `CheckboxGroup`, `DatePicker`, `Input`, `WheelPicker`, `DrawingPad`, `Slider`).
...
| `DatePicker` | — | `variableName`; `defaultValue`/`minimumDate`/`maximumDate` accept an ISO date string or the literal `"now"` (resolved to current date/time at render); `mode` (`date`/`time`/`datetime`); `format` = `Intl.DateTimeFormatOptions` subset (`hour`/`minute`/`day`/`month`/`year`/`hour12`/`dateStyle`/`timeStyle`/…) controlling the displayed/stored label per mode — e.g. `{hour:"2-digit",minute:"2-digit",hour12:false}` → `"14:30"`; don't mix `dateStyle`/`timeStyle` with component fields (Intl throws); see `DatePickerElement.ts` |
...
| `DrawingPad` | — | freehand draw/signature canvas; serializes per stroke into `variableName` (SVG path string) and/or `imageVariableName` (base64 image data URI — set either or both); `strokeColor`, `strokeWidth` (>0, default 2), `backgroundColor`, `clearable` (default true), `imageFormat: "png"\|"jpeg"` (default `"png"`); clear-button styling via `clearButtonPosition` (4 corners, default `"top-right"`) / `clearButtonOffset` / `clearButtonSize` / `clearButtonColor` / `clearButtonIconColor` / `clearButtonLabel`; box defaults height 200 / borderWidth 2 / borderRadius 16 / borderColor primary; needs `@shopify/react-native-skia`; see `DrawingPadElement.ts` |
| `Slider` | — | continuous numeric input — stores its value as a **float string** (`kind:"float"`) in `variableName` (read `{{var}}` in a `Text` `mode:"expression"`); `defaultValue` (defaults to `min`, else 0), `min` (default 0), `max` (default 1), `step` (0 = continuous; default 0); wire `minimumTrackTintColor`/`maximumTrackTintColor`/`thumbTintColor` from Design Profile; `disabled`; needs `@react-native-community/slider`; see `SliderElement.ts` |
...
| `TypewriterText` | — | reveals `content` one character at a time, each char mounting its own reanimated entering animation (per-char delay = `delay + charIndex * stagger`) — the staggered-headline reveal, **never writes a variable**. `content` (required), `mode` (`"plain"`/`"expression"` for `{{var}}`), `preset` (entering preset or `"none"` to disable, default `"FadeInDown"`), `duration` (400), `delay` (0), `stagger` (45 — the typewriter knob), `easing`, `spring: {damping?,stiffness?,mass?}` (wins over `easing`), plus text styling. Distinct from the whole-block `animation.entering` (fades the string at once) and from `AnimatedText` (an animated number); wraps by whole words; leaf, non-interactive; see `TypewriterTextElement.ts` |
