...
| `Image` | `url` (string) | NOT `source.uri`. WebP/AVIF decode via `expo-image` when installed (falls back to RN `Image`); `.svg` URLs auto-render via `react-native-svg` (`resizeMode`→`preserveAspectRatio`) — no schema change. Optional `blurRadius` (uniform px blur). `urlByVariable: {variableName, cases, default?}` swaps the source by a variable's value |
| `ProgressiveBlurImage` | `url`, `intensity` (0–100), `mask` | Full-bleed image with gradient-masked blur (sharp→blurred). `mask` is **linear** `{ from, to, stops }` or **radial** `{ type:"radial", center?:{x,y}, radius?, stops }`; each stop `{position 0–1, opacity 0–1}` where opacity = blur strength. Optional `tint` (`light`/`dark`/`default`), `maxBlurOpacity`. Bottom layer of a `ZStack` with sharp content above. Needs optional `@react-native-masked-view/masked-view` + `expo-linear-gradient` (linear) / `react-native-svg` (radial, bundled) + `expo-image`; degrades to sharp image + dark scrim |
| `Lottie` | `source` (string URL) | |
| `Rive` | `url` (string URL) | |
| `Input` | — | `variableName`, `keyboardType`, `autoCapitalize`, `maxLength`, `textAlign`. No `suffix`/`autoFocus`. |
| `Button` | `label` | `actions: ButtonAction[]` (`continue`/`custom`/`setVariable`/`navigate`/`openURL`/capability arms — see compose-screen-builder skill); `disabledWhen` (LeafCondition/Group); `pressedStyle`/`disabledStyle` (Partial overrides) + `transitionDurationMs`; `haptic` (`"none"\|"light"\|"medium"\|"heavy"\|"soft"\|"rigid"` — tactile feedback on press, opt-in, needs optional `expo-haptics`); shadow (`shadowColor`/`shadowOffset`/`shadowOpacity`/`shadowRadius`/`elevation`); explicit `backgroundColor`/`color`/`fontFamily`/`borderRadius` from Design Profile. `disabledStyle` supersedes deprecated `disabledBackgroundColor`/`disabledColor` |
| `RadioGroup` / `CheckboxGroup` | `items: [{label,value}]` | Items may add `description`/`imageUrl`/`imageAspectRatio`/`icon`. Wire `itemBackgroundColor`, `itemSelectedBackgroundColor`, `itemBorderColor`, `itemBorderRadius`, `itemFontFamily`, etc., from Design Profile; `columns` (1–6) makes an image-tile grid (+ `itemAlign:"column"`, `itemImageSize`); `showTick` (default `true`) hides the indicator when `false`; `CheckboxGroup` adds `minSelection`/`maxSelection`/`disableAtMax`; `haptic` fires on select/toggle (opt-in, needs `expo-haptics`) |
| `Slider` | `min`, `max` (`max>min`) | `variableName` (stores snapped value as string); `step`(>0)/`defaultValue`(in range)/`minLabel`/`maxLabel`/`showValue`/`valueSuffix`/`trackColor`/`trackFilledColor`/`trackHeight`/`knobColor`/`knobSize`; optional `reaction:{type:"Lottie"\|"Rive"\|"Image", height?, source? \| stops?:[{atValue,source}]}` (exactly one of source/stops); see `SliderElement.ts` |
| `Reviews` | `reviews: [{numberOfStar(0–5),content,authorName}]` | `showAward`/`awardLabel`/`awardStars`/`starColor`/`emptyStarColor`/`showOthersCount`/`cardBackgroundColor`/`layout`(`"card"\|"stack"\|"carousel"`); needs `react-native-svg` (bundled); see `ReviewsElement.ts` |
