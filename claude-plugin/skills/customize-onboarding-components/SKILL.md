...
- reveal: `preset` (entering preset applied per character, default `"FadeInDown"`; `"none"` disables the per-char animation), `duration` (per-char length, default 400), `delay` (lead-in before the first char, default 0), `stagger` (gap between consecutive chars — the typewriter knob, default 45), `easing` (default `"ease-out"`, ignored when `spring` set), `spring` (`{damping?, stiffness?, mass?}` — reanimated `.springify()`, wins over `easing`)
...
Containers (`YStack` / `XStack` / `ZStack` / `SafeAreaView` / `ScrollView` / `KeyboardAvoidingView` / `Carousel` / `RichText` / `Repeat`):
