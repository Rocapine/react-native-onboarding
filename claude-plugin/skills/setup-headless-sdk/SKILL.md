...
- **A change applies to the NEXT serve, never retroactively.** The onboarding is served once, when `OnboardingProvider` mounts, and that payload is frozen for the presentation: a `setUserProperty` during the flow does not refetch, re-key or swap the onboarding (and never blanks the screen) — it is picked up on the next mount or launch. So write a property the moment you compute it, even mid-onboarding; there is no need to seed it before mount. A paywall is served at `register`/`present`, so it follows the store until that call. To re-serve an onboarding with the current properties, remount the provider (a `key`).
