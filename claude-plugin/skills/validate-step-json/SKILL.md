...
4. If `tsx` unavailable, fall back to structural check:
   ...
   - `props.onPress` (any element — it's a `BaseBoxProps` field) is a `ButtonAction[]` with the same entry shapes as `Button.props.actions`. Schema-valid on every element; runtime ignores it on the elements that own their own gesture (`Button` — use `actions`; `RadioGroup`, `CheckboxGroup`, `DatePicker`, `Input`, `WheelPicker`, `Slider`) — warn if seen there, error only on a malformed action entry
   ...
   - **Flow-level chain integrity (when input is an array of steps)**:
     ...
     - Every variable referenced in a `branches[].condition` is captured upstream (by an Input / RadioGroup / CheckboxGroup / DatePicker / WheelPicker / Slider `variableName` or a `setVariable` action) — warn if not.
