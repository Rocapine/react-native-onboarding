...
4. If `tsx` unavailable, fall back to structural check:
   ...
   - `ProgressiveBlurImage.props` requires `url` (string), `intensity` (0–100), and `mask` (`{from, to, stops: [{position 0–1, opacity 0–1}]}`, ≥2 stops). Optional `tint` ∈ `"light"|"dark"|"default"`, `maxBlurOpacity` 0–1. Not a container — must NOT have `children`. Mask `opacity` is blur strength (not a color); a color `mask` is a schema error
