...
4. If `tsx` unavailable, fall back to structural check:
   ...
   - `RadioGroup.props.items` / `CheckboxGroup.props.items` is `[{label?, value, subLabel?}]` (NOT `options`) — `value` required + unique; `label` optional (omit for a sub-label-only row); `subLabel` optional secondary line. Tick styling: `tickPosition` (`"start"|"end"`), `tickColor`, `tickSelectedColor`, `tickBorderRadius`, `tickSize` (diameter/box side px, default `20`); sub-label styling: `itemSubLabel{Color,FontSize,FontWeight,FontFamily,FontStyle}` + `itemSelectedSubLabelColor`
