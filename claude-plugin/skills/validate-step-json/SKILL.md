...
4. If `tsx` unavailable, fall back to structural check:
   ...
   - `ProgressiveBlurImage.props` requires `url` (string), `intensity` (0–100), and `mask`. The `mask` is a union: **linear** `{from, to, stops}` (optional `type:"linear"`; `from`/`to` are edges) **or** **radial** `{type:"radial", center?:{x,y} 0–1, radius?>0, stops}`. Both need ≥2 stops `{position 0–1, opacity 0–1}`. Optional `tint` ∈ `"light"|"dark"|"default"`, `maxBlurOpacity` 0–1. Not a container — must NOT have `children`. Mask `opacity` is blur strength (not a color); a color `mask` is a schema error. A radial mask without `from`/`to` is valid — don't flag it
   - `Lottie.props.source` is a string; `Rive.props.url` is a string
   - `RadioGroup.props.items` / `CheckboxGroup.props.items` is `[{label, value}]` (NOT `options`); items may also carry `description?`/`imageUrl?`/`imageAspectRatio?`/`icon?`. Optional group props: `columns` (1–6 int, grid), `itemImageSize` (>0), `itemAlign` (`"row"|"column"`), `itemDescriptionColor`/`itemDescriptionFontSize`. `CheckboxGroup` adds `minSelection` (≥0 int) / `maxSelection` (≥1 int) / `disableAtMax` (bool) — `maxSelection` must be `≥ minSelection` (schema error otherwise)
   - `Slider.props` requires `min`/`max` numbers with `max > min` (schema error otherwise); `step` (if present) > 0; `defaultValue` (if present) within `[min, max]`. Optional `reaction: { type: "Lottie"|"Rive"|"Image", height?, source?, stops? }` must provide **exactly one** of `source` or `stops` (both/neither = schema error); each `stops[].atValue` must be within `[min, max]`. Not a container — must NOT have `children`
   - `Reviews.props.reviews` is a non-empty `[{ numberOfStar (0–5), content, authorName }]`. Optional: `showAward`/`showOthersCount` (bool), `awardLabel`, `awardStars` (0–5), `starColor`/`emptyStarColor`/`cardBackgroundColor`, `layout` ∈ `"card"|"stack"|"carousel"`. Not a container — must NOT have `children`
   - `Image.props.urlByVariable` (if present) is `{ variableName (non-empty), cases: Record<string,string>, default? }`
   ...
   - `Button.props.actions` is an array; entries are `"continue"` or a discriminated-union object on `type`: `custom` (`{function,variables?}`), `setVariable` (`{name,value,valueMode?,kind?}`; `valueMode` ∈ `"literal"|"expression"`, `kind` ∈ `"int"|"float"|"string"`), `navigate` (`{stepId}` — non-empty), `openURL` (`{url,external?}` — non-empty url), or a capability arm (`requestNotificationPermission` / `requestHealthSync` `{metrics?}` / `presentPaywall` `{placement?}` / `restorePurchase` / `requestReview`, each `{writes?: [{name,kind?,label?}]}`). All arms are in the headless discriminated union (`ButtonElement.ts`); an unknown `type` is a schema error
