...
4. If `tsx` unavailable, fall back to structural check:
   ...
   - `Button.props.actions` is an array; entries are `"continue"` or `{type:"custom",function,variables?}` or `{type:"setVariable",name,value,valueMode?,kind?}` (`valueMode` ∈ `"literal"|"expression"`; `kind` ∈ `"int"|"float"|"string"` — all three action shapes are the `ButtonAction` union in the headless schema, `common.types.ts`, re-exported from `ButtonElement.ts`)
