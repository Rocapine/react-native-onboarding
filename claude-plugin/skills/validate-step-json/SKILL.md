...
4. If `tsx` unavailable, fall back to structural check:
   ...
   - `RadioGroup.props.items` / `CheckboxGroup.props.items` is `[{label, value}]` (NOT `options`). Optional tick-styling props: `tickSize` (>0), `tickBorderWidth`/`tickBorderRadius` (≥0), `tickBorderColor`/`tickSelectedBorderColor`/`tickBackgroundColor`/`tickSelectedBackgroundColor`/`tickColor` (strings) — any other type is a schema error
   ...
   - `onClick` (optional, on ANY element via `BaseBoxProps`) is a **single** `ButtonAction` — `"continue"` or `{type:"custom",…}` or `{type:"setVariable",…}` (same shape as a `Button.actions` entry, but one action, not an array). A bad action shape fails parse.
