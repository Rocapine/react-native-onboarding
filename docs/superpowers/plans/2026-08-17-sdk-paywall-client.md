...
- **The rendering engine does not change.** `renderElement.tsx` and `ScreenRenderer.tsx` are host-agnostic already; a paywall is a second caller, not a modification. A task that finds itself editing either has taken a wrong turn — stop and report.
  ...
  **Two deliberate exceptions, both in `Runtime/ScreenHost.ts` and both owned by Task 4**, because the seam's *contract* genuinely widens for a second host: `complete` gains an optional outcome argument, and an optional `presentPaywall` field is added. `ScreenHost.ts` is the seam, not the engine — widening it is the mechanism by which the engine stays untouched. `elements/actions.ts` and `elements/runActions.ts` also change in Task 4; they are the action layer, not the renderer.
