...
- **The settled signal is a duration, injectable by the host** via the new optional `ScreenHost.enteringSettleDelayMs` (default `DEFAULT_ENTERING_SETTLE_MS`, 350ms — a typical native-stack push). It is not a framework signal because none is usable: React Native **stubbed `InteractionManager`** — as of 0.85 `runAfterInteractions` is a bare `setImmediate` and `createInteractionHandle()` returns `-1` — so anything built on it resolves on the next tick and defers nothing. RN's `Image` never registered an interaction handle in any version either, so it would not have covered decode even unstubbed. The host is the only party that knows its own navigator's transition duration, which is why the knob lives on `ScreenHost`.
