...
- **New `ProgressiveBlurImage` ComposableScreen element** — a full-bleed image with a gradient-masked Gaussian blur baked in (sharp where the `mask` is transparent, progressively blurred where it's opaque — the "welcome screen" hero look). Props: `url`, `intensity` (0–100, maps to a blur radius), `tint` (`light`/`dark`/`default`), `mask` (`{ from, to, stops: [{ position, opacity }] }`, linear; mask `opacity` = blur strength), `maxBlurOpacity`, plus standard box props. Leaf element (no `children`); intended as the bottom layer of a `ZStack`. (UI renders this by masking a blurred copy of the image — see the UI changelog.)
