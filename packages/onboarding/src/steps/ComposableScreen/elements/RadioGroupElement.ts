...
export type RadioGroupElementProps = BaseBoxProps & {
  ...
  /** Tick (radio circle) size in px. Default 20. */
  tickSize?: number;
  /** Tick border width. Default 2. */
  tickBorderWidth?: number;
  /** Tick border radius. Default = tickSize/2 (a circle). Lower it for a rounded-square tick. */
  ...
  /** Selected tick fill. Default transparent (radio). */
  tickSelectedBackgroundColor?: string;
  /** Inner dot color when selected. Default theme primary. */
  tickColor?: string;
};
