...
action counts as a way off the screen only when the resolve path AND the
unregistered-handler path each reach one (`completingActions.ts`), the same AND
rule `requestPermission` uses. A `"continue"` TRAILING the action satisfies both
of those, so `[{custom}, "continue"]` does read as a way forward — the throw path
is the only one that skips it, and a throw with the retries spent is something
the user can retry by pressing again. `onError: ["continue"]` next to the
trailing one covers that last path too, and is worth writing for a handler that
can fail offline.
