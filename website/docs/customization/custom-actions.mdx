...
| **Retry** | None by default. `retry: { maxAttempts }` re-invokes the same handler with the same press-time variables, up to `maxAttempts` total attempts; `delayMs` pauses between them, `timeoutMs` bounds each one. |
