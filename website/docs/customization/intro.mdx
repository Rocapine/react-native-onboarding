...
- Async-aware — chain awaits `Promise` returns; a throw runs the action's `onError` and the chain carries on, so gate navigation with `onResolve` rather than by throwing
