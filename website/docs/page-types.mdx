...
If that first visible render is the screen's **initial mount**, the play is *deferred* until the screen has settled rather than suppressed — an entrance that fires during the host navigator's push transition is half-consumed by it. Deferring keeps the animation; suppressing would have replaced a half-played entrance with none. The wait is a duration (default 350ms, overridable by the host via `ScreenHost.enteringSettleDelayMs`) rather than a framework signal, because React Native stubbed `InteractionManager` and offers nothing else to observe. It therefore covers the entry transition but **does not** wait on remote images to decode — a cold, slow first load can still outrun it. Later visibility flips never replay, and `once` wins over `replayWhen` when both are set.
