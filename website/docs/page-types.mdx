...
| `Image` | `<Image>` | — | — |
| `ProgressiveBlurImage` | `<Image>` + masked `BlurView` | depth (z-axis) | `expo-blur`, `@react-native-masked-view/masked-view`, `expo-linear-gradient` (all optional) |
...
|------|------|-------|
| `url` | `string` | **Required** — remote image URI |
| `intensity` | `number` | **Required** — blur strength, `0`–`100` (expo-blur intensity) |
...
The progressive blur needs `expo-blur`, `@react-native-masked-view/masked-view`, and `expo-linear-gradient` (all optional). When any is missing, the element degrades to a **sharp image + a dark gradient scrim** derived from the mask (still legible for overlaid text) and never throws. The `mask` is linear-only; a radial Figma mask is approximated by a vertical fade.
:::
