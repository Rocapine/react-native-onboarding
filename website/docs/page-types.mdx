...
`ComposableScreen` is still under active development. The element schema and renderer API may change before being considered stable.
:::

Build arbitrary onboarding screens entirely from CMS data — no custom renderer required. You define a tree of layout and text elements that is rendered directly to native `View` and `Text` components.
