...
If that first visible render is the screen's **initial mount**, the play is *deferred* until the screen has settled rather than suppressed — an entrance that fires during the host navigator's push transition is half-consumed by it. Deferring keeps the animation; suppressing would have replaced a half-played entrance with none. The wait is a duration (default 350ms, overridable by the host via `ScreenHost.enteringSettleDelayMs`) rather than a framework signal. `InteractionManager.runAfterInteractions` fails in two opposite ways — stubbed in RN 0.85+ so it fires immediately, and on earlier versions its queue reportedly does not drain while `react-native-screens` push transitions are active so it fires late or never — and nothing else is observable. **Treat the default as a starting point:** if an entrance still reads early, raise the delay before suspecting the mechanism. It therefore covers the entry transition but **does not** wait on remote images to decode — a cold, slow first load can still outrun it. Later visibility flips never replay, and `once` wins over `replayWhen` when both are set.
