...
- `{ type: "purchase", product: string, onSuccess?: ButtonAction[], onCancel?: ButtonAction[], onError?: ButtonAction[] }` — buys a resolved store product. `product` is a product slot key (e.g. `"yearly"`) or an interpolable `{{var}}` ref — typically the value a `RadioGroup` bound to `variableName: "plan"` just wrote. Runs `onSuccess` when the purchase completes, `onCancel` when the user backs out, `onError` on a store-level failure; a `"pending"` result (Ask-to-Buy / deferred transaction) runs none of them — the host app owns that case. All three follow-up lists are optional. **Caveat when `product` is a `{{var}}` ref:** interpolation resolves to the variable's `label` when set, falling back to `value` only if there's no label — so a `RadioGroup` item authored as `{ value: "yearly", label: "Yearly" }` resolves `{{plan}}` to `"Yearly"`, not `"yearly"`, and the purchase silently fails to find a matching product. Give such an item the same string for both `value` and `label` (e.g. `{ value: "yearly", label: "yearly" }`) if it feeds a `purchase` action this way.
