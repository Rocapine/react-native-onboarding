...
| `mask` | `{ from, to, stops: [{ position, opacity }] }` | **Required** — linear gradient driving where the blur is strong. `from`/`to` are edges (`"top"`, `"bottom"`, …); each stop's `opacity` (0–1) is the **blur strength** at `position` (0–1), not a color |
...
The progressive blur masks a **blurred copy** of the image (not a backdrop `BlurView` — a masked `BlurView` has no backdrop to sample and renders transparent on iOS). It needs `@react-native-masked-view/masked-view` + `expo-linear-gradient` (+ `expo-image` for the blurred copy), all optional. When the masked-view native module is missing (incl. a dev-client built before it was installed), the element degrades to a **sharp image + a dark gradient scrim** derived from the mask (still legible for overlaid text) and never throws. The `mask` is linear-only; a radial Figma mask is approximated by a vertical fade.
:::
