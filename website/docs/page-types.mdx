...
**Key features:**
- Recursive element tree (`YStack`, `XStack`, `ZStack`, `SafeAreaView`, `Text`, `RichText`, `Image`, `Lottie`, `Rive`, `Icon`, `Video`, `Input`, `RadioGroup`, `CheckboxGroup`, `Button`, `DatePicker`, `WheelPicker`, `Slider`, `ProgressIndicator`, `AnimatedText`, `Carousel`)
...
- Lottie, Rive, Video, DatePicker, WheelPicker, and Slider as optional peer deps — graceful fallback if not installed
- **Variable context** — `Input`, `RadioGroup`, `CheckboxGroup`, `DatePicker`, `WheelPicker`, and `Slider` elements write into shared state; `Text` elements interpolate those values with `{{variableName}}` expressions
...
`Input`, `RadioGroup`, `CheckboxGroup`, `DatePicker`, `WheelPicker`, and `Slider` elements write into a shared `composableVariables` map stored in `OnboardingProgressContext`. This state persists across navigation between `ComposableScreen` steps, so values collected on an early screen are available on later screens.
