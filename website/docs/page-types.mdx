...
Runtime **ignores `onPress`** on elements that already own a tap / focus / scroll gesture: `Button` (use `actions`), `RadioGroup`, `CheckboxGroup`, `DatePicker`, `Input`, `WheelPicker`. The schema still accepts it everywhere (it lives on `BaseBoxProps`), so wire selection / continue through those elements' own props instead.
