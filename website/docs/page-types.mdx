...
**Key features:**
- Recursive element tree (`YStack`, `XStack`, `ZStack`, `SafeAreaView`, `Text`, `Image`, `Lottie`, `Rive`, `Icon`, `Video`, `Input`, `RadioGroup`, `CheckboxGroup`, `Button`, `DatePicker`, `WheelPicker`, `Carousel`)
...
**Dependencies:** None for `YStack`, `XStack`, `ZStack`, `Text`, `Image`, `Icon`, `Input`, `RadioGroup`, `CheckboxGroup`, `Button`. `SafeAreaView` requires `react-native-safe-area-context` (already a peer dep of `@rocapine/react-native-onboarding-ui`). See the Carousel, Lottie, Rive, Video, DatePicker, and WheelPicker sections above for elements with peer deps.
