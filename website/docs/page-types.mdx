...
**Key features:**
- Recursive element tree (`YStack`, `XStack`, `ZStack`, `SafeAreaView`, `Text`, `RichText`, `Image`, `Lottie`, `Rive`, `Icon`, `Video`, `Input`, `RadioGroup`, `CheckboxGroup`, `Button`, `DatePicker`, `WheelPicker`, `DrawingPad`, `Slider`, `ProgressIndicator`, `AnimatedText`, `TypewriterText`, `Carousel`, `Repeat`)
...
| `Repeat` | Materializes one template per row of `props.data`; layout-transparent | — | — |
...
**`animation.entering.once`** — play the entrance **exactly once per screen lifetime**, on the first render where the element is visible.

Reach for it on anything gated by `renderWhen`. A false gate returns `null`, so `renderWhen` visibility is mount/unmount and reanimated fires `entering` on mount — meaning a gated element replays its entrance every time the gate flips back to true. There is no payload-level workaround: `gte` still unmounts when you move backwards past the threshold, and `replayWhen` is the exact opposite (it remounts on *every* change).

If that first visible render is the screen's **initial mount**, the play is *deferred* until the screen has settled rather than suppressed — an entrance that fires during the host navigator's push transition is half-consumed by it. Deferring keeps the animation; suppressing would have replaced a half-played entrance with none. The wait is a duration (default 350ms, overridable via the `enteringSettleDelayMs` prop on `OnboardingPage`, or on `ScreenHost` for a custom host) rather than a framework signal. `InteractionManager.runAfterInteractions` fails in two opposite ways — stubbed in RN 0.85+ so it fires immediately, and on earlier versions its queue reportedly does not drain while `react-native-screens` push transitions are active so it fires late or never — and nothing else is observable. **Treat the default as a starting point:** if an entrance still reads early, raise the delay before suspecting the mechanism. It therefore covers the entry transition but **does not** wait on remote images to decode — a cold, slow first load can still outrun it. Later visibility flips never replay, and `once` wins over `replayWhen` when both are set.

**`animation.replayWhen`** — a variable NAME. Re-fires `entering` whenever that variable's value changes, so an element re-animates **without** disappearing first (previously only toggling `renderWhen` could replay one). It works by remounting the element's subtree, so transient state inside resets and a continuous `effect` restarts — use it on presentational subtrees rather than around an `Input`. The initial mount is not a replay.

**`inset`** — declarative absolute placement, honoured on **`ZStack` children only**:

```
{ top?, left?, right?, bottom? }   // number = px, string = percentage of the stack ("62.1%")
```

A `ZStack` wraps each child in an absolutely-positioned full-bleed box and applies one shared `justifyContent`/`alignItems` to all of them, so the 9 anchor points are otherwise the only declarative placement. `inset` replaces hand-computed `transform.translateX/translateY` for off-anchor layers.

An omitted side is **not** `0` — that axis falls back to the stack's shared anchor, so `{ top: 40 }` pins vertically while still honouring the horizontal anchor. Supplying one side per axis (`top` + `left`) keeps the element content-sized and pinned by that corner; supplying both sides of an axis resolves to a size and stretches it. When an axis is positioned its shared anchor is dropped for that child, so placement is correct at every anchor, not just `flex-start`. Prefer percentages — px offsets authored against one screen width drift on others. Composes with `transform`; inert outside a `ZStack`.
...
Every UIElement also inherits an optional `onPress: ButtonAction[]` from `BaseBoxProps` — the **same action list as `Button.actions`** (`"continue"`, `{ type: "setVariable", … }`, `{ type: "custom", … }`, `{ type: "purchase", … }`, `{ type: "restore", … }`, `{ type: "dismiss" }`, `{ type: "presentPaywall", … }`, `{ type: "requestPermission", … }`, run sequentially, `"continue"`/`"dismiss"` terminal; see the **Button actions** section below). Use it to make a static element react to a tap — a tappable card/`Image`, a logo that advances the flow, a `Text` row that fires a custom action.
...
| Prop | Type | Notes |
|------|------|-------|
| `url` | `string` | **Required** — remote image URI. In `mode: "expression"`, `{{variable}}` interpolation runs first |
| `mode` | `"plain" \| "expression"` | `expression` enables `{{var}}` interpolation in `url`, so one element can serve a data-driven set (`.../zodiac/{{zodiacSign}}.png`) instead of one duplicated subtree per case. References resolve to the variable's **`value`**, not its `label` — a URL segment is a machine identifier, so `{ value: "aries", label: "Aries" }` yields `aries`. An unknown variable interpolates to an empty string, so keep a fallback segment or a `renderWhen` guard. Defaults to `"plain"`, which stays fully static and does not subscribe to variable writes |
...
- `{ type: "custom", function: string, variables?: string[], onResolve?: ButtonAction[], onError?: ButtonAction[], retry?: { maxAttempts: number, delayMs?: number, timeoutMs?: number } }` — invokes a developer-injected handler registered on `OnboardingProvider.customActions`. `variables` lists the keys to read from the live ComposableScreen variable map and forward to the handler. The handler is awaited, so an async one gates the flow. `onResolve` runs when its promise resolves; `onError` runs when it fails (a throw with every attempt spent, an attempt past `retry.timeoutMs`, or an unregistered `function` name). NEITHER is terminal — the enclosing list carries on after both, as it does for `purchase`/`restore` — so an async gate's `"continue"` belongs inside `onResolve`, where it runs only on success, while a `"continue"` after the action advances on every outcome. `retry.maxAttempts` is the TOTAL attempts counting the first (1..10, required when `retry` is present; absent `retry` means one attempt), `delayMs` is a fixed pause between them (0..10000), and `timeoutMs` bounds ONE attempt (1000..300000, no default — absent means unbounded). **A retried handler must be idempotent**: every attempt re-invokes the same function with the same press-time variables, and a timed-out attempt is an ordinary failed attempt, so a request that reached the server and lost only the answer to the clock is sent again — `retry` on a create-or-charge endpoint is N writes for one press. While the list is awaiting, the runtime publishes `actions.pending` and `actions.pending.<elementId>` as `"true"` in the variable map, so `renderWhen`/`disabledWhen` can show a pending state with no host code; a second press on the same element is dropped while it runs — on a `Button` and on the generic `onPress` of any element alike. Inside a `Repeat`, spell the per-element key with the TEMPLATE id (`actions.pending.row-cta`); each row resolves it to its own materialized element. A `custom` action counts as a way off the screen only when the resolve path (`onResolve` or anything after the action) AND the failure path (`onError` or anything after the action) each reach a `"continue"`/`{type:"dismiss"}`. `[{custom}, "continue"]` satisfies both; `{custom, onResolve:["continue"]}` on its own satisfies neither.
- `{ type: "setVariable", name: string, value: string, label?: string, valueMode?: "literal" | "expression", kind?: "int" | "float" | "string", arrayOp?: "append" | "remove" | "toggle" }` — writes to the variable map (`{ value, label }`). Pair with `"continue"` to set a discriminator a downstream `nextStep` rule can branch on; the value is visible to the same-tick branch evaluation. With **`valueMode: "expression"`** the `value` is evaluated rather than stored verbatim. Beyond `+ - * /` and parens it has a function stdlib: **numeric** `min(a,b,…)` `max(a,b,…)` `abs(a)` `round(a[,digits])` `clamp(a,lo,hi)`; **dates** `addDays(date,n)` `format(date,spec[,locale])` — `date` is an ISO string (what `DatePicker` stores) or the `"now"` sentinel, and `spec` is `DatePicker`'s Intl `format` vocabulary, either a bare `dateStyle` name (`"medium"`) or `key:value` pairs (`"weekday:long, day:numeric"`), NOT a `YYYY-MM-DD` token language; **listing** `list(x[,conjunction])` ("A, B and C") `join(x[,separator])` `count(x)` `plural(n,one,other)` — `x` is an untagged multi-select variable and its member LABELS are used (an explicit `kind:"string"` is taken at its word and read as raw values). A scalar answer counts as one member, also by its label. A variable holding a NUMBER is not a list — `count({{age}})` fails rather than answering 1, because `plural({{age}}, ...)` is what that wanted — and neither is a value that parses as JSON without being a `string[]` (`"[1,2,3]"`, `{"a":1}`). String literals are quoted, and a literal's CONTENTS interpolate — `list({{goals}}) + " for {{name}}"` reads "… for Ada", it does not emit the braces. **It runs at PRESS time only** — there is no mount hook and `Text mode:"expression"` interpolates rather than evaluating, so a computed headline must be written to a variable by an earlier press (usually the previous screen's Continue) and then plainly interpolated. A template that attempts a call and fails stores the EMPTY STRING and warns, so a broken `format(...)` shows as a blank, not as literal source text. An UNSET variable reads as `0` wherever it is data (so `{{counter}} + 1` works before the first press, and `count({{skipped}})` is 0) but is REFUSED as a `clamp` bound or a `round` digit count — `clamp({{score}}, {{floor}}, {{ceiling}})` with unseeded bounds used to report 0 for a score of 42. The taint follows the VALUE, so it also refuses one that reached a bound through arithmetic or a function — `addDays({{d}}, {{weeks}} * 7)`, `clamp({{trialDays}}, 1, 90)` — but it is dropped where the result could have come from an untainted argument: `max({{trialDays}}, 7)` is the explicit default it looks like, as is `max({{seeded_zero}}, {{absent}})`. A `{{ref}}` inside a quoted LITERAL is guarded the same way when it is configuration — `join({{goals}}, "{{sep}}")` with `sep` unset used to run the members together — while `count()` is deliberately exempt, since `count({{skipped}})` is a real zero. "Attempts a call" means a stdlib name — **or ANY unknown identifier glued to a `(`**, not just a close misspelling — sits in front of a `(` whose contents could actually BE arguments. Prose keeps its fallback to plain interpolation, so `"{{n}} day(s)"` still reads "3 day(s)", `"{{n}} min(s) left"` reads "3 min(s) left" (a bare word between the parens means they are punctuation, not a call), and `"Goals ({{n}})"` reads "Goals (2)" (`Goals` is not a stdlib name, and an unglued `(` after an UNKNOWN identifier is never a call). But `"Save(50)"`, `"Total({{n}})"` and `"Step({{n}}/3)"` DO blank and warn, because the identifier is glued to the paren and no bare word appears anywhere — one space (`"Save (50)"`) is the fix **for an unknown name only**. A stdlib name is called whether or not the paren is glued, so `"max (2)"` evaluates to `"2"`; text that must stay text needs `valueMode: "literal"`. A stdlib name whose whole template takes a bare-word argument, `count(goals)` with the braces forgotten, keeps its text and warns instead, since it is indistinguishable from `min(s)`. There is NO implicit concatenation, though: `list({{goals}}) and more` is a FAILED call (blank + warning), not prose — write `list({{goals}}) + " and more"`. With **`arrayOp`** the target is treated as a `CheckboxGroup`-style multi-select (JSON-encoded `string[]`) and `value`/`label` are the single member to `append` (add, dedup), `remove` (drop), or `toggle` (flip — matches a checkbox tap); the stored label stays comma-joined like a real checkbox and `kind` is ignored. This lets any element (via `onPress`) add or remove a chip from a multi-select without a `CheckboxGroup` widget.
- `{ type: "purchase", product: string, onSuccess?: ButtonAction[], onCancel?: ButtonAction[], onError?: ButtonAction[] }` — buys a resolved store product. `product` is a product slot key (e.g. `"yearly"`) or an interpolable `{{var}}` ref — typically the value a `RadioGroup` bound to `variableName: "plan"` just wrote. A `{{var}}` ref resolves to the variable's underlying `value` (the machine key), not its `label` — so a `RadioGroup` item authored as `{ value: "yearly", label: "Yearly" }` (nicer display text, distinct machine value — the normal pattern) still resolves `{{plan}}` to `"yearly"` and finds the right product, regardless of what the label says. Runs `onSuccess` when the purchase completes, `onCancel` when the user backs out, `onError` on a store-level failure; a `"pending"` result (Ask-to-Buy / deferred transaction) runs none of them — the host app owns that case. All three follow-up lists are optional.
- `{ type: "restore", onSuccess?: ButtonAction[], onNothingToRestore?: ButtonAction[], onError?: ButtonAction[] }` — restores prior purchases. Runs `onSuccess` when a purchase is restored, `onNothingToRestore` when the user had nothing to restore, `onError` on failure. All three follow-up lists are optional.
- `{ type: "dismiss" }` — finishes the current screen with a `{ status: "dismissed" }` outcome. In a paywall (see [Paywalls](./getting-started.mdx#paywalls)), this resolves the pending `present()` call — upgraded to `"purchased"`/`"cancelled"` instead if a purchase actually completed during this presentation. Outside a paywall (an onboarding step), it behaves like an early, unconditional exit from the screen.
- `{ type: "presentPaywall", placement: string }` — asks the host to present a paywall by placement (see [Paywalls](./getting-started.mdx#paywalls)). Works from an onboarding step or from a paywall's own content alike (a paywall can open another paywall). Not terminal — the action chain continues after it. Warns via `console.warn` and does nothing if no `PaywallProvider` is mounted anywhere above the host.
- `{ type: "requestPermission", kind: PermissionKind, onGranted?: ButtonAction[], onDenied?: ButtonAction[], onUnavailable?: ButtonAction[] }` — asks the OS for a permission, then runs the branch matching the answer. The hooks are ordinary nested `ButtonAction[]`, recursed exactly like `purchase`'s, which is what lets ONE press advance on a grant and do something different on a refusal — `[{ type: "custom" }, "continue"]` could ask and advance, but a custom handler's return value is discarded, so a grant and a refusal could only diverge on a later screen through a variable the handler wrote.

  `kind` is one of six, each asked through an **optional** Expo peer dep the app installs itself (nothing is bundled):

  | `kind` | needs |
  |---|---|
  | `notifications` | `expo-notifications` |
  | `appTrackingTransparency` | `expo-tracking-transparency` |
  | `locationWhenInUse` | `expo-location` (foreground only) |
  | `camera` | `expo-camera` |
  | `microphone` | `expo-audio`, or `expo-camera` |
  | `photoLibrary` | `expo-image-picker`, or `expo-media-library` |

  Not terminal in itself — put `"continue"` inside a hook to advance. `onGranted` or `onDenied` with no actions declared logs a `console.warn` rather than silently doing nothing. `onUnavailable` is the "this build cannot ask" branch — the module is not installed, or the platform has no such permission. Omitted, the runtime does **not** run `onDenied` in its place: nobody refused anything, so writing a refusal (a `setVariable`, an analytics event, a `renderWhen` gate that later reads it) would record a decision the user never made. Instead, if the author put a `"continue"` / `{dismiss}` anywhere in the ask — i.e. the press was the screen's way forward — the runtime **completes the screen with that same action**, preferring `{dismiss}` when the ask can reach both, and logs a `console.error` naming the missing module. Reusing the author's own outcome matters on a `Paywall` step: a bare `"continue"` is what that step's hard gate reads as "advance", so an ask authored with `{dismiss}` on both branches keeps the gate shut instead of handing out the gated content. If the ask was never a way forward (a "turn on notifications" button beside its own Skip CTA), nothing runs beyond that error. A declared `onUnavailable` always wins, including an empty one.

  Asking twice does not re-prompt: iOS shows its system dialog once and every later request resolves immediately with the standing answer — but the matching hook still runs, so a second press repeats whatever it does.

  **`requestPermission` on the host is an override for the six kinds above, not a way to add a seventh.** A host passes its own resolver via `OnboardingPage`'s `requestPermission` prop (or `PaywallHost`'s, for a paywall presented through `present()`) and returns `undefined` for any kind it does not handle, which falls back to the bundled Expo resolver. Use it when the app already owns a permission flow for one of these kinds — its own notification opt-in, say — and wants the authored CTA to run that instead.

  **HealthKit and Screen Time / Family Controls are NOT reachable this way today.** Both need entitlements and a config plugin that belong to the app rather than to a library, so they are not members of `kind` — and because `kind` is a closed enum, a payload naming `"healthKit"` or `"screenTime"` fails `invalid_union` and takes the whole screen to the error boundary before any resolver is consulted. Adding them means adding the kind to the schema first: `react-native-onboarding#261`. Until then, a host-owned permission of that sort stays a `{ type: "custom" }` action.

  **Reading a permission's current status is not available.** This is a press action; there is no way to skip a screen because the permission was already granted, and no variable carries the outcome to a later step.

  **Requires an installed SDK that ships this action — check before publishing one.** No version number is quoted here on purpose: the release that carries it is decided when it is cut, so a hardcoded floor in this page would be wrong in the dangerous direction (understating it green-lights publishing to an audience that loses the screen). The check that cannot go stale is the app's own copy: `grep -o requestPermission node_modules/@rocapine/react-native-onboarding/dist/steps/common.types.js` — no match means that build rejects the action. The forward-compat strip described under [Forward compatibility](#composablescreen) is keyed to unknown *element types*; `ButtonAction` is a plain union, so an app whose SDK predates the action fails the payload with `invalid_union` and the **whole screen** falls back to the error boundary — no CTA, and no back chevron on a `displayProgressHeader: false` step. Nothing yet transmits the installed SDK version to Studio (`react-native-onboarding#233` / `onboarding-studio#313`), so shipping this action to an audience that may be on an older build is a capability-floor decision rather than a safe default. Giving the runtime the same tolerance for an unknown *action* that it has for an unknown element is tracked as `react-native-onboarding#262`; until that lands, an older build loses the screen rather than the action.

Execution rules:

- Sequential. Each action awaits the previous before starting.
- Async-aware. If a custom handler returns a `Promise`, the runtime awaits it. `purchase`/`restore` always await the store round-trip before moving on.
- `"continue"` and `"dismiss"` are **terminal** — any actions after either are ignored.
- Every way a custom handler can fail is one rule: a throw with the retry budget spent, an attempt past `retry.timeoutMs`, or a `function` name with no matching key in `customActions`. All three log via `console.error`, run `onError` if it is declared, and let the rest of the chain run — the same as `purchase`/`restore`. Put what must only happen on success in `onResolve`.
- Without `retry.timeoutMs`, a handler whose promise never settles is awaited for the life of the screen: `actions.pending.<elementId>` stays `"true"`, so a CTA gated on it with `disabledWhen` never comes back.
- `purchase`/`restore` need a `ProductProvider` configured on `OnboardingProvider` (or, for a paywall, on `PaywallProvider` — see below). Without one — or when the runtime resolves `{ status: "error" }` and no `onError` list is declared — the action logs via `console.warn` instead of failing silently.

**Selling with `purchase` / `restore`**

Resolved store products are projected into the same variable map as flat dotted keys, so prices interpolate like any other variable: `product.<slot>.price`, `product.<slot>.pricePerDay`, `product.<slot>.pricePerWeek`, `product.<slot>.savingsPct`, `product.<slot>.trialDays`, plus `products.loaded` / `products.purchasing` / `products.error`. **Never author a price into the payload** — always read it from these variables, and gate the plan list / buy button on `products.loaded` so nothing is tappable before products resolve:
...
```jsonc
{
  ...
  "props": {
    "label": "Start free trial — {{product.yearly.price}}",
    "actions": [
      {
        "type": "purchase",
        "product": "yearly",
        "onSuccess": ["continue"],
        "onError": [{ "type": "custom", "function": "trackPurchaseError" }]
      }
    ]
  }
}
...
**Distinguishing "still loading" from "failed to load."** `products.loaded` is `"false"` for BOTH cases — it only means the store round-trip hasn't reached `"ready"` yet, not why. A paywall that treats the two as one thing shows a permanent spinner instead of telling the user something actually broke, which is worse than the App-Review-rejection risk the `products.loaded` gate above exists to avoid. Key the failure branch on `products.error` being non-empty instead, and the loading branch on `products.error` still being empty so it doesn't also match the failed case:
...
```jsonc
{
  ...
  "props": { "content": "We couldn't load pricing. Please check your connection and try again later." }
}
```

There is no `ButtonAction` to retry loading products — the runtime has no `reload()` (tracked as a follow-up in the spec). A failed load can only offer `dismiss` and rely on the host re-presenting the paywall later; don't paper over the gap with a `{ type: "custom" }` action naming a function no host implements.

**Guarding a purchase against a double-tap.** `products.purchasing` is `"true"` for the duration of the store round-trip a `purchase` action started. Disable the buy button for that window with `disabledWhen`, and pair it with a visible label explaining why — a disabled button with no explanation looks broken, and a label with no `disabledWhen` doesn't actually stop the second tap:
...
```jsonc
{
  ...
  "props": { "content": "Completing your purchase…" }
}
```

Both patterns ship as seeded templates in Onboarding Studio's template palette (**"Paywall — pricing block with failure state"** and **"Paywall — purchase button with in-flight state"**) and are exercised together in the SDK's own `example/app/example/paywall.tsx`.
...
| `preset` | entering preset | Reanimated entering preset applied per character (same set as `animation.entering.preset`: `FadeIn`/`FadeInUp`/`FadeInDown`/`SlideIn*`/`ZoomIn*`/`BounceIn*`/`FlipIn*`/etc., plus `"none"` to disable the per-character animation); defaults to `"FadeInDown"` |
...
| `reserveSpace` | `boolean` | Lay the fully-revealed text out invisibly up front and overlay the animation on it, so the element never grows mid-reveal and never pushes siblings down. Only meaningful with `cursor` (without it every character is mounted from frame 0, so the box is already stable). Because it measures the real resolved string it stays correct per locale — unlike a hardcoded wrapper height. Defaults to `false` |
...
Materializes its `children` (one template) once per row of `data`. It renders no view of its own — the rows become direct children of the enclosing container, so that container's `gap`, direction and alignment apply per row exactly as if the rows had been written by hand. For that reason its props do **not** extend `BaseBoxProps`.

| Prop | Type | Notes |
|------|------|-------|
| `data` | `Array<Record<string, string \| number \| boolean>>` | **Required.** One record per materialization, authored in the payload. Flat scalars only — nested objects aren't addressable as `{{item.x}}` and are rejected |
| `as` | `string` | Prefix the row's fields are exposed under; defaults to `"item"` |
| `keyField` | `string` | Row field used for each materialized element's id suffix and React key (`card` → `card__aries`). Falls back to the row index. Prefer setting it — ids stay meaningful and identity survives a reorder of `data` |

Each row's fields are readable as `{{item.<field>}}` and as `renderWhen` variables (`item.index` is always present). **A `renderWhen` on the template gates per row, so `Repeat` also covers the "show exactly one of N" case — there is no separate `Match` element.** The scope reaches press-time actions too, so a repeated card can write its own row's value with `{ "type": "setVariable", "valueMode": "expression", "value": "{{item.id}}" }`; scoped values are read-only, and every row's child writes the same variable *name*, distinguishing itself by the value.

`data` is authored in the payload rather than sourced from a variable holding JSON, so copy and asset URLs stay in the studio where designers and the translation pipeline are. For the same reason a translatable row string carries its own **literal** i18n key (`"titleKey": "zodiac_aries_title"`), never a computed one — key coverage is measured by scanning payloads for literal key strings, so a computed key reports the screen fully translated while its rows ship untranslated.
...
| `defaultIndex` | `number \| null` | Slide shown on mount; only applied at mount |
| `variableName` | `string` | Writes the **settled** slide index to the variable store on snap (`0`, `1`, `2`) — use for branching, analytics and `{{var}}` text |
| `progressVariableName` | `string` | Publishes the **continuous** swipe position as a screen-scoped animated variable, updated every frame, so sibling elements can gate `renderWhen` on the finger rather than on the settled slide. Always normalized to `[0, childCount)`: the underlying library value is unbounded when `loop` is on (lap two would report `3`, `4`, …), so the raw value would silently break gates after the first lap. Note `eq` compares the **rounded** value, so `eq 1` flips once the swipe passes halfway. UI-thread only — it is not written to the store, so `{{var}}` text cannot read it |
...
**`in` / `not_in` take a list.** Three shapes mean the same list, so a
membership gate can be authored against a literal, or against another
variable's selection:

```jsonc
{ "variable": "item.value", "operator": "not_in", "value": ["health", "sport"] }
{ "variable": "item.value", "operator": "not_in", "value": "{{selected}}" }   // multi-select variable
{ "variable": "item.value", "operator": "not_in", "value": ["{{selected}}"] } // same, array-wrapped
```

A `CheckboxGroup` stores its selection as a JSON-encoded array string, and a
reference to it is decoded back into a list before the comparison. Members
compare stringified, so a numeric `Repeat` row field matches a string member
(`1` is in `["1","2"]`), and `contains` against an array variable follows the
same rule. An **empty** list — and a reference to a variable nobody has written,
in either shape — has no members: `in` matches nothing, `not_in` matches
everything. A
right-hand side that is no list at all (`"operator": "in", "value": "male"`)
reads as a one-member list and logs a warning; author `["male"]` or use `eq`.
...
**Forward compatibility (unknown element types):** the element schema only knows the element types shipped in *your installed* SDK version. When a screen published from Studio uses a newer element type, the SDK **omits that element and its children** and renders the rest of the screen — it no longer fails the whole screen — and logs a `console.warn` naming the type and its path in the tree. Everything else is still validated strictly, so a genuine payload error (an enum value that doesn't exist, a missing `id`) still surfaces as a parse error rather than a quietly missing element. If omitting an element leaves nothing on the screen that can advance the step — the continue button is authored *inside* the element tree, so a newer element used as the screen's root container takes the CTA down with it — the SDK renders its own **Continue** button and logs a `console.error`, so nobody is stranded on a screen with no way out. Two practical consequences: upgrade the SDK to render a newly added element type, and treat that fallback button as a safety net rather than a design, since the screen the user sees is still missing whatever the element was for. `KNOWN_ELEMENT_TYPES` is exported from `@rocapine/react-native-onboarding` if you want to check a payload against the installed build. One nuance on "can advance the step": a `requestPermission` action counts as a way forward only when a grant **and** a non-grant both reach a `"continue"`/`dismiss` — a CTA holding its only `"continue"` in `onGranted` reads as no way forward, because a refusal (or a build without the optional module) would strand the user.
