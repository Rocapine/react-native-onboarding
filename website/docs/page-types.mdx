...
- `{ type: "custom", function: string, variables?: string[], onResolve?: ButtonAction[], onError?: ButtonAction[], retry?: { maxAttempts: number, delayMs?: number } }` — invokes a developer-injected handler registered on `OnboardingProvider.customActions`. `variables` lists the keys to read from the live ComposableScreen variable map and forward to the handler. The handler is awaited, so an async one gates the flow. `onResolve` runs when its promise resolves and is NOT terminal (the enclosing list carries on); `onError` runs when it throws with every attempt spent, and IS terminal — a throw always aborts the rest of the list, hook or no hook, so an async gate's `"continue"` belongs inside `onResolve` rather than after the action. `retry.maxAttempts` is the TOTAL attempts counting the first (1..10, required when `retry` is present; absent `retry` means one attempt) and `delayMs` is a fixed pause between them (0..10000). While the list is awaiting, the runtime publishes `actions.pending` and `actions.pending.<elementId>` as `"true"` in the variable map, so `renderWhen`/`disabledWhen` can show a pending state with no host code; a second press on the same element is dropped while it runs — on a `Button` and on the generic `onPress` of any element alike. Inside a `Repeat`, spell the per-element key with the TEMPLATE id (`actions.pending.row-cta`); each row resolves it to its own materialized element. A `function` name the host never registered is an error outcome: it logs, runs `onError` if declared, and the rest of the list still runs — so a `custom` action counts as a way off the screen only when the resolve path (`onResolve` or anything after the action) AND the unregistered-handler path (`onError` or anything after the action) each reach a `"continue"`/`{type:"dismiss"}`. `[{custom}, "continue"]` satisfies both; `{custom, onResolve:["continue"]}` on its own satisfies neither.
